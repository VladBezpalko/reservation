...
def test_validate_not_it_past(fixt_date_in_the_past):
    with pytest.raises(ValidationError):
        validate_not_in_past(fixt_date_in_the_past)


def test_validate_not_late(fixt_date_late_to_reserve):

    with pytest.raises(ValidationError):
        validate_not_late(fixt_date_late_to_reserve)


def test_validate_not_weekend(fixt_date_weekend):
    with pytest.raises(ValidationError):
...
def test_all_validators_accept_valid_date(fixt_normal_date):
